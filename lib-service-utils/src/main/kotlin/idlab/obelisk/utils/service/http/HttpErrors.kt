    ...
    override fun accept(err: Throwable?) {
        when (err) {
            ...
            is LimitExceededException -> {
                ctx.writeErrorJson(429, err.message)
            }
...
private fun RoutingContext.writeErrorJson(status: Int, message: String? = null, details: List<String>? = null) {
    val errorJson =
        io.vertx.kotlin.core.json.json { obj("error" to JsonObject.mapFrom(ErrorOutput(status, message, details))) }
