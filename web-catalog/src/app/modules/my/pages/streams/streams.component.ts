...
import { CreateStreamInput, Dataset, DataStream, EventField, EventFieldTuple, EventField_ALL, FilterExpressionSchema, TimestampPrecision, TimestampPrecisionTuple, TimestampPrecision_ALL } from '@shared/model';
...
export class StreamsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.streamSource = new ObeliskDataSource(this.obelisk.listMyStreams.bind(this.obelisk), { filterFn: filter })
  }
  ...
  addStream(comp: CreateHeaderComponent<any>) {
    ...
    this.obelisk.createStream(input)
  ...
  endStream(id: string, event?: MouseEvent) {
    this.confirm.areYouSureThen(
      ...
      this.obelisk.endStreamSession(id)
    )
      .subscribe(resp => this.respHandler.observe(resp, {
        success: _ => {
