...
import { ConfirmService, ResponseHandlerService, RoleService, ToastService } from '@core/services';
...
import { Client, DataRemovalRequestInput, Dataset, FilterExpressionSchema, Metric, Permission, RestrictionInput } from '@shared/model/types';
...
export class ClientComponent implements OnInit {
  ...
  private loadClient(clientId: string) {
    this.obelisk.getClient(clientId).subscribe(client => {
      this.client = client;
  ...
  deleteData() {
    if (this.removalForm.valid) {
      ...
      const actualObs = defer(() => this.confirm.areYouSureThen('Are you really sure? The deleted data cannot be restored!', this.obelisk.removeClientData(this.client.id, request)))
      this.confirm.areYouSureThen('Are you sure you want to <b>permanently</b> delete all of this client\'s data? <b>This is irreversible!!</b>', actualObs)
