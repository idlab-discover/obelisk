...
import { Dataset, EventField, EventFieldTuple, EventField_ALL, Export, ExportInput, FilterExpressionSchema, Team, TimestampPrecision, TimestampPrecisionTuple, TimestampPrecision_ALL } from '@shared/model';
...
@UntilDestroy()
@Component({
  selector: 'app-team-exports',
  templateUrl: './team-exports.component.html',
  styleUrls: ['./team-exports.component.scss']
})
export class TeamExportsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.route.data.pipe(
      ...
      this.exportSource = new ObeliskDataSource(this.obelisk.listTeamExports.bind(this.obelisk, team.id), { filterFn: filter });
    });
  }
  ...
  addExport(comp: CreateHeaderComponent<any>) {
    ...
    this.obelisk.createTeamExport(this.team.id, input).subscribe(resp => {
  ...
  removeExport(id: string, event?: MouseEvent) {
    this.confirm.areYouSureThen(
      "Are you sure you want to remove this export?",
      this.obelisk.removeTeamExport(this.team.id, id)
    ).subscribe(resp => {
      if (resp.responseCode === 'SUCCESS') {
  ...
  get filterActive() {
    return JSON.stringify(this.exportForm?.get('filter')?.value) != '{}';
  }
